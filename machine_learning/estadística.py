...
#————————————————————————————————————————————————————————————————————————————————————————————————————————————————————————————————————————————
def estadística_B(B: np.ndarray) -> np.ndarray:
  """
  ...
  discontinuidades y coherencia del salto, entre otras. Devuelve una lista de 9 elementos.
  """
  if len(B) < 3:                                 # Si len(B) es al menos 3, puedo obtener coherentes gradientes, percentiles, etc.
    return np.zeros(9, dtype=np.float32)         #=> si hay 2 puntos o menos no aporta valor físico => devuelvo 0.
  ...
  res = [                                        # En la variable res, voy a agregar (appendear) toda la estadística de |B|:
    B.mean(),                                    # la media,
  ...
  return np.round(res, 4).astype(np.float32)     # Devuelvo res redondeado en 4 dígitos después de la coma para mayor rapidez.

def estadística_componentes_B(B_i: np.ndarray) -> np.ndarray:
  """
  ...
  otras. Devuelve una lista de 8 elementos.
  """
  if len(B_i) < 3:                                   # Si len(x) >= 3, puedo obtener coherentes gradientes, percentiles, etc.
    return np.zeros(8, dtype=np.float32)             #=> si hay 2 puntos o menos no aporta valor físico => devuelvo 0.
  ...
  res = [                                            # En la variable res, agrego toda la estadística de la componente B_i:
...
#————————————————————————————————————————————————————————————————————————————————————————————————————————————————————————————————————————————
def estadística_R(R: np.ndarray) -> np.ndarray:
  """
  ...
  bow shock. Devuelve una lista de 10 elementos.
  """
  if len(R) < 3:                                # Mientras haya al menos 3 puntos, puedo obtener coherentes gradientes, percentiles, etc.
    return np.zeros(10, dtype=np.float32)       # => si hay 2 puntos o menos no aporta valor físico => devuelvo 0.
...
def estadística_componentes_R(x_i: np.ndarray) -> np.ndarray:
  """
  ...
  mediciones poco bruscas (suaves) son útiles para aportar contexto al algoritmo KNN. Devuelve una lista de 7 elementos.
  """
  if len(x_i) < 3:                      # Mientras haya al menos 3 puntos, puedo obtener coherentes gradientes, percentiles, etc.
    return np.zeros(7, dtype=np.float32)# => si hay 2 puntos o menos no aporta valor físico => devuelvo 0.
  x_izq, _, x_der = dividir_ventana(x_i)# Obtengo el primer y el último tercio de ventana, respectivamente (la dividí en 3).
  res = [                               # En la variable res, agrego toda la estadística de la componente x_i:
    x_i.mean(),                         # la media,
