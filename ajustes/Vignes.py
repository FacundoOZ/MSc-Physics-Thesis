...
def hipérbola_Vignes(
    *,
    ...
    x_0: float = 0.64,          # ± 0.02 [R_m]        # Offset (desplazamiento) en el eje Xss (por Vignes).
...
def función_hipérbola_Vignes(
    ...
    *,
    ...
    referencia SS (Xss) donde ocurrió un bow shock, y recibe un parámetro float x_0. Utilizando todos los parámetros standard dados por
    Vignes (2000), excepto x_0, traza la hipérbola única con aberración de 4° (como Vignes) para el intervalo dado por las mediciones x_datos
    cuyo offset es x_0, y la devuelve en formato y=f(x) solo para y>0 (devuelve y). El parámetro 'cant_puntos' permite ajustar la definición de
    la hipérbola creada.
    """
    _,_,x_a,y_a = hipérbola_Vignes(x_0=x_0,cant_puntos=cant_puntos)# Construyo la cónica de Vignes con el x_0 y la cantidad de puntos propuestos.
    ...
    return y_modelo                                                # Devuelvo la hipérbola Vignes de forma y=f(x) que ajustó mejor x_0 y x_datos.
#————————————————————————————————————————————————————————————————————————————————————————————————————————————————————————————————————————————
...
def hipérbola_máxima() -> tuple[np.ndarray, np.ndarray]:
  ...
  """
  _, _, x_a_max, y_a_max = hipérbola_Vignes(θ0= 0.41, θf=2.0, x_0=1.40, α=-np.deg2rad(10))# Obtengo solo las coordenadas aberradas
...
def hipérbola_mínima() -> tuple[np.ndarray, np.ndarray]:
  ...
  """
  _, _, x_a_min, y_a_min = hipérbola_Vignes(θ0=-0.39, θf=1.9, x_0=0.13, α= np.deg2rad(20))# Obtengo solo las coordenadas aberradas
...
def mínimo_2019() -> tuple[float, float]:
  ...
  cil = módulo(-706.178000, -5353.465000, norm=R_m) # Obtengo la proyección de las componentes yz, en el plano vertical y también normalizo.
